...
import { uploadWorkOrderPhotosAction } from "@/lib/factory-actions";
...
/** צירוף סקיצות ותמונות ייחוס להזמנת עבודה, שהמפעל יראה. */
export default function WorkOrderPhotoUploader({ workOrderId }: { workOrderId: string }) {
  const [pending, setPending] = useState<Pending[]>([]);
  const [busy, setBusy] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);
  const [, formAction, sending] = useActionState(async (_: null, fd: FormData) => {
    await uploadWorkOrderPhotosAction(fd);
    setPending([]);
    return null;
  }, null);
